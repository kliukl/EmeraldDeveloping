...
"""
function layer_1_sif_vec!(τ_i_θ::FT, τ_i_12::FT, τ_i_21::FT, k_e::FT, τ_θ::FT, ρ_1::FT, ρ_2::FT, f_sife::FT, k_f::SubArray, vec_b::SubArray, vec_f::SubArray) where {FT}
    ...
    @. vec_f = f_sife / 2 * k_e * (eꜜ * int_exp_kef⁻(k_f, k_e) + eꜛ * int_exp_kef⁺(k_f, k_e));
    @. vec_b = f_sife / 2 * k_e * (eꜛ * int_exp_kef⁻(k_e, k_f) + eꜜ * int_exp_kef⁺(k_f, k_e));
...
"""
function layer_2_sif_vec!(k_e::FT, τ_θ::FT, ρ_1::FT, ρ_2::FT, τ_2::FT, f_sife::FT, k_f::SubArray, vec_b::SubArray, vec_f::SubArray) where {FT}
...
# Bug Fix:
#     2025-May-22: fix an issue related to forward and backward SIF emission to make sure it is the same as the Dualspect when N = 2
#
#######################################################################################################################################################################################################
"""
...
leaf_sif_matrices!(config::SPACConfig{FT}, bio::LeafBio{FT}, cache::SPACCache{FT}, ::PlatespectFluorescenceSpectra) where {FT} = (
    ...
    for i in eachindex(IΛ_SIFE)
        ...
        vec_f_1_out .= layer_sif_out.(vec_b_1, vec_f_1, ρ_21_sif, τ_all_sif_1);
        vec_b_1_out .= layer_sif_out.(vec_f_1, vec_b_1, ρ_21_sif, τ_all_sif_1);
        vec_f_2_out .= layer_sif_out.(vec_b_2, vec_f_2, ρ_21_sif, τ_all_sif_2);
        vec_b_2_out .= layer_sif_out.(vec_f_2, vec_b_2, ρ_21_sif, τ_all_sif_2);
